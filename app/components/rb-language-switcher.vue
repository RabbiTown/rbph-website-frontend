...
<template>
  ...
  <rb-confirm-modal
    ...
    :title="warningTitle"
    :confirm-label="t('common.switchLanguageConfirm')"
    confirm-color="warning"
    confirm-icon="material-symbols:translate-rounded"
    @confirm="confirmLocale"
  >
    <template #body>
      <div class="space-y-3">
        <p>{{ targetWarning }}</p>
        <p v-if="locale !== pendingLocale" class="text-muted">{{ currentWarning }}</p>
      </div>
    </template>
  </rb-confirm-modal>
</template>
