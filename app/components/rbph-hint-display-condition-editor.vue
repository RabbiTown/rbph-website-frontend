...
function updateModel() {
  updatingModel = true;
  model.value = serializeUnlockGate(gate.value) ?? '(true)';
  nextTick(() => {
    updatingModel = false;
  });
}
...
async function loadOptions() {
  loading.value = true;
  try {
    const [puzzleResponse, roundResponse] = await Promise.all([useApi().get<{ puzzles: AdminPuzzleData[] }>(`/admin/puzzles?game_id=${props.gameId}`), useApi().get<{ rounds: AdminRoundData[] }>(`/admin/rounds?game_id=${props.gameId}`)]);
...
<template>
  <rbph-unlock-condition-editor v-model="gate" :puzzles="puzzles" :rounds="rounds" :disabled="disabled" :loading="loading" :allow-default="false" hint-states @change="updateModel" />
</template>
