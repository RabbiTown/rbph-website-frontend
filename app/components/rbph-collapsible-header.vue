...
<template>
  <div class="group flex items-start gap-3" :class="{ 'cursor-pointer': collapsible }">
    <div class="flex min-w-0 flex-1 flex-wrap items-start gap-x-3 gap-y-2">
      <div ref="title" class="min-w-[min(100%,14rem)] grow-[999] shrink basis-56 whitespace-normal [overflow-wrap:anywhere]">
        <slot />
      </div>
      <div v-if="$slots.badges || $slots.actions" ref="controls" class="flex min-w-0 max-w-full grow basis-auto flex-wrap items-center gap-2 [&_button]:max-w-full [&_a]:max-w-full [&_span]:whitespace-normal [overflow-wrap:anywhere]" @click.stop @keydown.stop>
