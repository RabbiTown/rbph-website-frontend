...
const { renderer } = useFrontendRenderer({
  published: () => round.value?.renderer,
  gameId: () => round.value?.data.game_id,
  surface: 'round-page',
  roundId: () => round.value?.data.id,
  previewRevision: () => Number(route.query.preview) || undefined,
});
...
const rendererCurrencies = computed<RbtCurrency[]>(() =>
  Object.values(currentCurrencies.value).map(currency => ({
    id: currency.id,
    slug: currency.slug,
    name: currency.name,
...
let fetchToken = 0;
async function updateData(clearExisting = true) {
  const token = ++fetchToken;

  if (!props.path) {
    showError('Invalid round route');
    return;
  }

  if (clearExisting) round.value = undefined;
  try {
    const { data } = await api.get<RbRoundUserData>(props.path);
  ...
  try {
    ...
    if (useHostToast) {
      ...
      const toastData = {
        ...
        description: penalty ? `${data.result.result || action.desc} ${penalty}` : data.result.result || action.desc,
        icon: action.icon,
        color: action.color,
        duration: 10000,
      };
...
const rendererActions = {
  ...
  listSubmissions,
  refresh: async () => updateData(false),
...
watch(
  () => [renderer.value?.mode, renderer.value?.manifestUrl, renderer.value?.rendererId],
  () => {
    rendererFailed.value = false;
  },
);
watch(
  () => useCustomRenderer.value && renderer.value?.layout === 'game-full',
  full => setPageLayout(full ? 'game-full' : 'game'),
  { immediate: true },
);
...
<template>
  <div v-if="round && useCustomRenderer && renderer">
    <rbph-page-renderer
      :renderer="renderer"
      :page-key="`round:${round.data.id}`"
      :page="round"
      :actions="rendererActions"
      :routes="rendererRoutes"
      :content-blocks="round.data.contents"
      :currencies="rendererCurrencies"
      @failed="rendererFailed = true"
    />
  </div>
  <div v-else-if="round">
    <div class="py-6">
