...
const submitLimitItems = [
  { label: '不限制', value: 'none', icon: 'material-symbols:all-inclusive-rounded' },
  ...
  <div v-if="puzzle" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,64rem)_minmax(0,1fr)]">
    ...
    <u-form :state="state" class="min-w-0 space-y-4" @submit.prevent="apply">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <u-button icon="material-symbols:add-rounded" label="添加规则" :disabled="saving" @click="addRule()" />
      </div>

      <div v-if="state.rules.length" class="space-y-3" @dragover="onRuleListDragOver" @drop="onRuleDrop">
        <div
        ...
        <div class="space-y-3 rounded-lg bg-elevated/60 p-4 ring ring-default">
          ...
          <div class="grid gap-3 md:grid-cols-[10rem_minmax(0,1fr)] md:items-center">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <u-input-number
                v-if="state.penalty.submitLimitType === 'limited'"
                v-model="state.penalty.maxSubmit"
                :min="0"
                :step="1"
                orientation="vertical"
                trailing="次"
