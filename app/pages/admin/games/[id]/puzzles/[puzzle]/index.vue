...
const { puzzle, contentEditor, focusTitle, headerDirty, applyHeader, resetHeader } = useAdmin().usePuzzleContext();
const ownerId = computed(() => puzzle.value?.id);
const blocks = useAdminContentBlocks('puzzles', ownerId);
...
async function apply() {
  ...
  try {
    await blocks.save();
    dirtyToast.clear();
    toast.add({ title: t('admin.pages.puzzle.index.puzzleContentSaved'), icon: 'material-symbols:check-rounded', color: 'success' });
  } catch (error) {
    handleError(error, t('admin.pages.puzzle.index.savePuzzleContentFailed'));
  }
}
...
watch(dirty, value => {
  if (value) dirtyToast.show({ description: t('admin.pages.puzzle.index.puzzleContentUpdateNotYetSave'), guardOnLeave: true, apply, reset });
...
<template>
  <div v-if="puzzle" class="grid min-h-0 gap-6 xl:grid-cols-[18rem_minmax(0,64rem)_minmax(18rem,22rem)]">
    <rbph-content-block-browser
      :blocks="blocks.blocks.value"
      :selected-id="blocks.selectedId.value"
      :game-id="puzzle.game_id"
      :current-puzzle-id="puzzle.id"
    ...
    <u-form v-if="blocks.selected.value" :state="blocks.selected.value" class="min-h-0 min-w-0" @submit.prevent="apply">
      <rbph-content-editor ref="contentEditor" v-model="blocks.selected.value.content" :placeholder="t('admin.common.enterContent')" :aria-label="t('admin.pages.puzzle.index.puzzleContent')" :disabled="blocks.saving.value" @focus-title="focusTitle" @save="apply" />
    </u-form>
