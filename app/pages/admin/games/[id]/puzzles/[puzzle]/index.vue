...
async function apply() {
  ...
  try {
    await blocks.save();
    dirtyToast.clear();
    toast.add({ title: '谜题内容已保存', icon: 'material-symbols:check-rounded', color: 'success' });
  } catch (error) {
...
watch(dirty, value => {
  if (value) dirtyToast.show({ description: '谜题内容修改尚未保存。', guardOnLeave: true, apply, reset });
...
<template>
  <div v-if="puzzle" class="grid min-h-0 gap-6 xl:grid-cols-[18rem_minmax(0,64rem)_minmax(18rem,22rem)]">
    ...
    <u-form v-if="blocks.selected.value" :state="blocks.selected.value" class="min-h-0 min-w-0" @submit.prevent="apply">
      <rbph-content-editor ref="contentEditor" v-model="blocks.selected.value.content" placeholder="请输入内容" aria-label="谜题正文" :disabled="blocks.saving.value" @focus-title="focusTitle" @save="apply" />
    </u-form>
