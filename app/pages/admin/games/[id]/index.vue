...
function resetCurrencyDraft(currency: AdminCurrencyData) {
  ...
  delete currencyDeletePending[currency.id];
}
...
function syncCurrencyDrafts(next: CurrencyItem[]) {
  ...
  for (const key of Object.keys(currencyDrafts)) {
    if (!next.some(currency => currency.id === Number(key))) {
      delete currencyDrafts[Number(key)];
      delete currencyDeletePending[Number(key)];
    }
  }
}
...
function removeLocalCurrency(currency: CurrencyItem) {
  ...
  delete currencyDrafts[currency.id];
  delete currencyDeletePending[currency.id];
}
...
async function submitChanges() {
  ...
  try {
    ...
    for (const currency of currencies.value.filter(item => !item.local && currencyDeletePending[item.id])) {
      await api.delete(`/admin/games/${current.id}/currencies/${currency.id}`, {
    ...
    for (const currency of currencies.value.filter(item => !item.local && currencyDirty.value[item.id] && !currencyDeletePending[item.id])) {
      const draft = currencyDrafts[currency.id];
      if (!draft) continue;
      await api.patch(`/admin/games/${current.id}/currencies/${currency.id}`, {
        ...draft,
    ...
    for (const currency of currencies.value.filter(item => item.local)) {
      const draft = currencyDrafts[currency.id];
      if (!draft) continue;
      await api.post(`/admin/games/${current.id}/currencies`, {
        ...draft,
        name: draft.name.trim(),
        slug: draft.slug.trim(),
      }, { errorHints: currencyErrorHints() });
    ...
    <div class="flex min-w-0 flex-col gap-4">
      <u-form :state="state" class="flex flex-col gap-4" @submit.prevent>
        ...
        <section class="space-y-4">
          ...
          <div v-if="!currencyLoading && currencies.length > 0" class="space-y-3">
            <template v-for="currency in currencies" :key="currency.id">
              <div
                v-if="currencyDrafts[currency.id]"
                class="relative transition-colors"
                :class="[
                  currencyDeletePending[currency.id] ? 'opacity-55 before:content-[\'\'] before:pointer-events-none before:absolute before:-start-4 before:top-0 before:bottom-0 before:w-0.5 before:rounded-full before:bg-error' : '',
                  !currencyDeletePending[currency.id] && currencyDirty[currency.id] ? 'before:content-[\'\'] before:pointer-events-none before:absolute before:-start-4 before:top-0 before:bottom-0 before:w-0.5 before:rounded-full before:bg-warning' : '',
                ]"
              >
                <div class="relative rounded-lg bg-elevated/60 px-4 py-3 ring ring-default">
                  <div class="absolute right-3 top-3 z-10 flex items-center justify-end gap-1">
                    <u-badge v-if="currency.local" color="primary" variant="soft">待创建</u-badge>
                    <u-badge v-else-if="currencyDeletePending[currency.id]" color="error" variant="soft">待删除</u-badge>
                    <u-button v-else-if="currencyDirty[currency.id]" size="xs" variant="soft" color="warning" class="group relative h-5 w-12 overflow-hidden px-0 text-[11px]" :disabled="currencySubmitting" @click="resetCurrencyChange(currency)">
                    ...
                    <u-button v-if="currencyDeletePending[currency.id]" icon="material-symbols:undo-rounded" color="neutral" variant="ghost" size="sm" :disabled="currencySubmitting" @click="restoreCurrency(currency)" />
                        ...
                        <div class="w-64 p-3 text-sm">
                          <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0">
                              ...
                              <div class="mt-1 text-xs text-muted">确认将「{{ currency.name }}」标记为待删除？保存后相关队伍余额、提示消耗和处罚配置会被清除或置空。</div>
                            </div>
                          </div>
                          <div class="mt-3 flex justify-end">
                            <u-button size="xs" color="error" variant="soft" icon="material-symbols:delete-outline-rounded" @click="markCurrencyDeleting(currency)">标记删除</u-button>
                          </div>
                        </div>
                      </template>
                    </u-popover>
                  ...
                  <div class="flex flex-col gap-3">
                    <div class="grid gap-3 md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
                      <rb-form-field label="名称">
                        <u-input v-model="currencyDrafts[currency.id].name" placeholder="例如 提示点" class="w-full" :disabled="currencySubmitting || currencyDeletePending[currency.id]" />
                      </rb-form-field>
                      <rb-form-field label="内部 ID">
                        <u-input v-model="currencyDrafts[currency.id].slug" placeholder="例如 hint" class="w-full font-mono" :disabled="currencySubmitting || currencyDeletePending[currency.id]" />
                      </rb-form-field>
                    </div>
                    <div class="grid gap-3 md:grid-cols-[7rem_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)]">
                      <rb-form-field label="精度">
                        <u-input-number v-model="currencyDrafts[currency.id].prec" :min="0" :max="6" :step="1" orientation="vertical" class="w-full" :disabled="currencySubmitting || currencyDeletePending[currency.id]" />
                      </rb-form-field>
                      <rb-form-field label="初始值">
                        <rb-input-number v-model="currencyDrafts[currency.id].init_amount" :prec="currencyDrafts[currency.id].prec" :min="0" :max="currencyDrafts[currency.id].max_amount" :step="10 ** currencyDrafts[currency.id].prec" orientation="vertical" class="w-full" :disabled="currencySubmitting || currencyDeletePending[currency.id]" />
                      </rb-form-field>
                      <rb-form-field label="增长/分钟">
                        <rb-input-number v-model="currencyDrafts[currency.id].growth" :prec="currencyDrafts[currency.id].prec" :step="10 ** currencyDrafts[currency.id].prec" orientation="vertical" class="w-full" :disabled="currencySubmitting || currencyDeletePending[currency.id]" />
                      </rb-form-field>
                      <rb-form-field label="上限">
                        <rb-input-number v-model="currencyDrafts[currency.id].max_amount" :prec="currencyDrafts[currency.id].prec" :min="0" :step="10 ** currencyDrafts[currency.id].prec" orientation="vertical" class="w-full" :disabled="currencySubmitting || currencyDeletePending[currency.id]" />
                      </rb-form-field>
                    </div>
                  </div>
                </div>
